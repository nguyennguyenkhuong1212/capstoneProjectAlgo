...
def plot_input_data(warehouses, customers):
    ...
    plt.scatter(coords_customers[:, 0], coords_customers[:, 1], s=30, c='b', label='customer')
    plt.scatter(coords_warehouses[:, 0], coords_warehouses[:, 1], s=60, c='r', label='warehouse')
...
def plot_clusters(warehouses, customers, centroids, clusters, cluster_indexes_to_show):
    ...
    # Plots the warehouse.
    plt.scatter(coords_warehouses[:, 0], coords_warehouses[:, 1], s=60, c='r', marker='s', label='warehouse')
...
def plot_assigned_customers(warehouses, vehicles, vehicle_indexes_to_show):
    ...
    plt.legend()
    plt.grid()
    plt.savefig(IMG_DIR + "assigned.png")
    plt.show()
...
def plot_vehicle_tour(vehicle, vehicle_tour):
    ...
    plt.scatter(vehicle_tour[-1].x, vehicle_tour[-1].y, s=60, c='r', marker='s', label='landfill')
    ...
    plt.legend()
    plt.grid()
    plt.savefig(IMG_DIR + "{0}.png".format(vehicle.index + 1))
    plt.show()
...
def greedy(points):
    ...
    dist = {int(point.index): float('infinity') for point in points}
    pre = {int(point.index): int(point.index) for point in points}
    print("AAA", dist)
    print("BBB", pre)
...
def plan_vehicle_routing(warehouse, landfill, vehicle):
    ...
    graph.append(1)  # Edge from the last vertex to the warehouse
